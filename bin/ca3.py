...
#boll
k_interval = "15m"  #k线周期 1m 3m 5m 15m 30m 1h 2h 4h 6h 8h 12h 1d 3d 1w 1M
boll_interval = 55 #布林周期
stddev = 2 # 标准差系数
...
class FutureTrader:
    ...
    def checkOpenOrders (self, orders, ticker_price):
        ...
        for i in range(len(arrNet)): 
            ...
            if check_order.get('status') == "CANCELED":
                ...
                self.logger.info('=====卖单=====  ' + json.dumps(sell_order))
                if sell_order:
                    arrNet[i]["state"] = "cover"
    ...
    def onTick (self) :
        ...
        for i in range(len(arrNet)):
            if i != len(arrNet) - 1 and arrNet[i]["state"] == "idle" and ticker_price > arrNet[i]["price"] and ticker_price <= arrNet[i + 1]["price"] and ticker_price >= boll_ma and ticker_price <= boll_up:
                ...
                self.logger.info('=====买单=====  ' + json.dumps(buy_order))
                if buy_order: 
